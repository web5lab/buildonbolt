...
export function useShareTemplate() {
  ...
  const shareTemplate = async (template: Template): Promise<ShareResponse> => {
    setIsSharing(true);
    try {
  ...
  const shareViaEmail = (template: Template) => {
    const shareUrl = getTemplateUrl(template);
  ...
  const shareViaSocialMedia = (platform: string, template: Template) => {
